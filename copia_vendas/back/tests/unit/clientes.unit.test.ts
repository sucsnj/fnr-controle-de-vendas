...
describe('Clientes - Testes Unitários (US08-US12)', () => {
  ...
  describe('US08 - Cadastrar Cliente', () => {
    ...
    test('Deve falhar ao cadastrar cliente com email duplicado', async () => {
      mockReq.body = { nome: 'João', cpfCnpj: '12345678900', email: 'joao@example.com', responsavel: 'João' }
      ;(prisma.cliente.findUnique as jest.Mock)
        .mockResolvedValueOnce(null) // cpfCnpj check
        .mockResolvedValueOnce({ id: 1, email: 'joao@example.com' }) // email check

      await createCliente(mockReq as any, mockRes as Response)

      expect(mockRes.status).toHaveBeenCalledWith(400)
      expect(mockRes.json).toHaveBeenCalledWith(
        expect.objectContaining({ detail: expect.stringContaining('E-mail já cadastrado') })
      )
    })
  })
